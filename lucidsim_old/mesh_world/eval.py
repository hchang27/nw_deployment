...
def process_image(img, device, transform=None):
    data = img[None, ...].permute(0, 3, 1, 2).contiguous()
...
class Eval(PrefixProto):
    ...
    rgb_checkpoint = "/alanyu/scratch/2024/01-27/205004/checkpoints/net_1000.pt"
    ...
    async def main(self, sess: VuerSession):
        ...
        sess.set @ DefaultScene(
            ...
            rawChildren=[
                ...
                Sphere(
                    ...
                    material=dict(side=2, color="blue"), # map=f"http://localhost:8012/static/{self.background_img}"),
        ...
        for step in range(self.num_steps):
            ...
            img_t = process_image(img_t, self.env.device, self.transform)

            # img_t = torch.rand_like(img_t, device=img_t.device)

            pred_actions, _ = self.policy(img_t, obs)
