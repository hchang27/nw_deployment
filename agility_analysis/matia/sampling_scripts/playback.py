...
from vuer.schemas import AmbientLight, TimelineControls, group, TriMesh, Scene, Urdf, Movable, DirectionalLight, PointLight
...
class PlayArgs(ParamsProto):
    ...
    checkpoint = "/lucid-sim/lucid-sim/debug/launch/2023-12-14/15.37.25/200"
    ...
    urdf = Proto(env="$HOME/mit/parkour/main_street/assets/robots/gabe_go1")


app = Vuer(static_root=PlayArgs.urdf, port=8013, uri="ws://localhost:8013")
...
# PlayArgs.checkpoint = "/lucid-sim/lucid-sim/baselines/launch_grav_teacher_stairs/2024-01-10/23.28.54/go1/700"
PlayArgs.checkpoint = "/lucid-sim/lucid-sim/baselines/launch_grav_teacher/2024-01-05/12.44.09/go1/600"
...
def set_env(args):
    assert (PlayArgs.checkpoint is None) ^ (PlayArgs.exptid is None), "Either logger_prefix or exptid must be not None"
    ...
    ppo_runner, train_cfg = task_registry.make_alg_runner(
        load_checkpoint=PlayArgs.checkpoint, env=env, name=args.task, args=args, train_cfg=train_cfg
    )
