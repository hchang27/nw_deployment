...
from torchvision.transforms.v2.functional import center_crop_image
...
class BallXYD(PrefixProto, cli=False):
    data_path = Proto(
        env="$DATASETS/lucidsim/lucidsim/experiments/matia/visuoservoing/ball_gen/ball-train-v3",
    ...
    def __post_init__(self, _deps=None):
        ...
        if image_path == "lucid_dreams":
            num_trials = len(image_list) / len(xyz)
            assert num_trials.is_integer(), "the number of trials should be an integer."
            num_trials = int(num_trials)
            xyz = xyz.repeat(num_trials, 1)
        assert len(data) == len(xyz), "the data should align."

        self.data = torch.permute(data, [0, 3, 1, 2]).contiguous()
        self.target = xyz.contiguous()
...
if __name__ == "__main__":
    ...
    dataset = BallXYD(transform=transform, device="cuda", debug=True)
