...
class TrainCfg(PrefixProto):
    ...
    # checkpointing
    checkpoint_interval: int = n_epochs // 2
...
def train(train_loader, model, optimizer):
    ...
    for i, (image, target) in enumerate(
        ...
        output = model(image_stacked)

        # target *= 100
        loss = position_loss = F.mse_loss(output[:, :1], TrainCfg.position_scale * target[:, :1])
...
def main(**deps):
    ...
    if TrainCfg.model_checkpoint is not None:
        ...
        model = get_soda_model(inp_shape=(2 * 3, 45, 80), device=TrainCfg.device, projection_dim=6,
                               coord_conv=False)
