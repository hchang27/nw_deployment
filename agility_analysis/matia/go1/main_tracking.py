...
class TrainCfg(PrefixProto):
    ...
    scenes = ["ball/debug/scene_00002"]
    # trajectories_per_scene = [[0, 1, 2, 3, 5, 7, 9, 11, 13, 15]]
    trajectories_per_scene = [list(range(15))]

    val_scenes = ["ball/debug/scene_00002"]
    val_trajectories_per_scene = [list(range(15, 18))]

    image_type: Literal["rgb", "depth", "augmented"] = "augmented_1"
    ...
    data_aug: List[
        Literal["crop", "rotate", "color", "perspective", "blur"]] = ["crop", "rotate", "color", "perspective"]
    ...
    # imu obs:
    teacher_checkpoint = "/lucid-sim/lucid-sim/scripts/train/2024-03-08/14.37.26/14.37.26/1"
...
def fetch_dreams():
    ...
    loader_class = StackedLucidDreams if (TrainCfg.use_stacked_dreams or TrainCfg.recurrent) else LucidDreams
...
def train(train_loader, model, criterion, optimizer):
    ...
    for i, (camera, obs, action) in enumerate(train_loader.sample_batch(TrainCfg.batch_size, shuffle=TrainCfg.shuffle)):
        ...
        # Symmetry Loss: if we flip the camera image horizontally, the yaw should be negated
        flipped_camera = torch.flip(camera, dims=[3])
        flipped_pred_actions, _, _, flipped_pred_yaw = model(flipped_camera, obs)

        symmetry_loss = (pred_yaw + flipped_pred_yaw).norm(p=2, dim=1).mean()
        ...
        loss = latent_loss + yaw_loss + TrainCfg.symmetry_coef * symmetry_loss
        ...
        logger.store_metrics({"train/symmetry_loss": symmetry_loss.item()})
...
def main(**deps):
    ...
    vision_head = get_soda_model(inp_shape=(num_channels, 45, 80), num_filters=TrainCfg.num_filters,
                                 ...
                                 num_shared_layers=TrainCfg.num_shared_layers, coord_conv=False)
    ...
    for epoch in range(0, TrainCfg.n_epochs + 1):
        ...
        if (
            ...
            logger.save_torch(model.state_dict(), f"checkpoints/state_dict_{epoch}.pt")
