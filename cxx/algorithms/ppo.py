...
from cxx.modules import ActorCritic
from cxx.storage import RolloutStorage
...
class PPO:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.num_scan = estimator_params["num_scan"]
        ...
        if self.if_depth:
            ...
            self.depth_encoder_optimizer = optim.Adam(self.depth_encoder.parameters(), lr=depth_encoder_params["learning_rate"])
            self.depth_encoder_paras = depth_encoder_params
            self.depth_actor = depth_actor
            self.depth_actor_optimizer = optim.Adam(
                [*self.depth_actor.parameters(), *self.depth_encoder.parameters()], lr=depth_encoder_params["learning_rate"]
            )
    ...
    def update(self):
        ...
        for (
            ...
            priv_reg_coef = (
                priv_reg_stage * (self.priv_reg_coef_schedual[1] - self.priv_reg_coef_schedual[0]) + self.priv_reg_coef_schedual[0]
            )

            # Estimator
            priv_states_predicted = self.estimator(obs_batch[:, : self.num_prop])  # obs in batch is with true priv_states
            estimator_loss = (
                (priv_states_predicted - obs_batch[:, self.num_prop + self.num_scan : self.num_prop + self.num_scan + self.priv_states_dim])
                .pow(2)
                .mean()
            )
