...
def main(args):
    ...
    logs_path_src = f"/home/{args.user}/a1-loco/main_street/logs/parkour_new"
    logs_path_dst = "/home/cxx/a1-loco/main_street/logs/parkour_new"

    folders = subprocess.check_output(
        ["ssh", f"cxx@{args.server}.pc.cs.cmu.edu", "cd", logs_path_src, "&&", "find", "*", "-maxdepth", "0", "-type", "d"]).decode("utf-8")
    ...
    models = subprocess.check_output(
        ["ssh", f"cxx@{args.server}.pc.cs.cmu.edu", "cd", exp_path_src, "&&", "find", "*", "-maxdepth", "0"]).decode("utf-8")
    models = models.split("\n")
