...
class LeggedRobotCfg(BaseConfig, PrefixProto, cli=False):
    cli = False

    class play(PrefixProto, cli=False):
        load_student_config = False
        mask_priv_obs = False

    class env(PrefixProto, cli=False):
        yaw_resampling_rate = 5
        num_envs = 6144
    ...
    class commands(PrefixProto, cli=False):
        ...
        # Easy ranges
        class ranges(PrefixProto, cli=False):
        ...
        # Easy ranges
        class max_ranges(PrefixProto, cli=False):
        ...
        class crclm_incremnt(PrefixProto, cli=False):
    ...
    class domain_rand(PrefixProto, cli=False):
        ...
        # this is the initial step where we turn on the action delay for the robot.
        delay_update_global_steps = 24 * 8000
        action_delay = False
        action_curr_step = [6, 6]
        action_curr_step_scratch = [0, 6]
        action_delay_view = 6
        action_buf_len = 24

    class rewards(PrefixProto, cli=False):
        class scales(PrefixProto, cli=False):
            # tracking rewards
...
class LeggedRobotCfgPPO(BaseConfig, PrefixProto, cli=False):
    seed = 1
    ...
    class depth_encoder(PrefixProto, cli=False):
        print("WARNING: these passing-through are problematic.")
    ...
    class estimator(PrefixProto, cli=False):
