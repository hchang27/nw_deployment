...
from isaacgym.torch_utils import quat_apply, torch_rand_float, quat_from_euler_xyz, to_torch, get_axis_params, quat_rotate_inverse
from tqdm import tqdm

from ...utils import wrap_to_pi, quat_apply_yaw

assert gymapi, "force import first to avoid torch error."
import torch  # noqa: E402
import torchvision  # noqa: E402
from torchtyping import TensorType  # noqa: E402

from main_street import MAIN_ST_ROOT_DIR
from main_street.envs.base.base_task import BaseTask
from main_street.utils.helpers import (
    euler_from_quaternion,
    smart_delta_yaw,
)

# from main_street.utils.math import *
from main_street.utils.terrain import Terrain
...
class LeggedRobot(BaseTask):
    def __init__(self, cfg: LeggedRobotCfg, sim_params, physics_engine, sim_device, headless):
        ...
        self.render_camera_props = gymapi.CameraProperties()
        render_camera = self.gym.create_camera_sensor(self.envs[0], self.render_camera_props)

    def render(self, mode: str = "rgb_array", env_id: int = 0, visualize_depth: bool = False):
        """render the simulated environment.

        Args:
            mode (): OneOf['rgb_array', 'logger']
            env_id (): index of the environment we want to render
            visualize_depth (): if true, returns third_person and depth (ego view)

        Returns:
            third_person (image)
            or
            third_person (image), depth (ego)
        """
        if mode == "logger":
            bx, by, bz = self.root_states[self.robot_actor_idxs[env_id], :3]
            ...
            self.gym.set_camera_location(
                camera_handle,
                self.envs[env_id],
                gymapi.Vec3(bx, by - 1, bz + 1),
                gymapi.Vec3(bx, by, bz),
            )

            # The call to env.step in the evaluator will do gym.simulate and gym.fetch_results
            # We step the graphics in this function to only render graphics during evaluations..
    ...
    def step(self, actions, **obs_kwargs):
        ...
        # render the third person camera. Noops when headless.
        self.render()

        for _ in range(self.cfg.control.decimation):
            # pass in the unclipped actions -> self.action_history_buf
        ...
        self.post_physics_step(**obs_kwargs)
        ...
        if self.cfg.depth.use_camera and not self.cfg.depth.ignore_vision and self.global_counter % self.cfg.depth.update_interval == 0:
            self.extras["depth"] = self.depth_buffer[:, 0]  # have already selected last one
        else:
    ...
    def process_depth_image(self, depth_image, env_id):
        ...
        # zero out random values on the depth map
        # depth_image += self.cfg.depth.dis_noise * 2 * (torch.rand(1) - 0.5)[0]

        blackout_idxs = torch.where(torch.rand(depth_image.shape, device=depth_image.device) < self.cfg.depth.dis_noise)
    ...
    def update_depth_buffer(self):
        if not self.cfg.depth.use_camera or self.cfg.depth.ignore_vision:
            return
    ...
    def _update_goals(self):
        # if it has been more than 0.1 sec since you reached the goal, move to the next and reset the timer
        ...
        self.reached_goal_ids = (
            torch.norm(self.root_states[self.robot_actor_idxs, :2] - self.cur_goals[:, :2], dim=1) < self.cfg.env.next_goal_threshold
        )
    ...
    def post_physics_step(self, **kwargs):
        ...
        self.step_external_assets(**kwargs)
        ...
        self.last_root_pos[:] = self.root_states[self.robot_actor_idxs, :3]
        ...
        if self.viewer and self.enable_viewer_sync and self.debug_viz:
            ...
            self._draw_goals()
            # self._draw_feet()
            if self.cfg.depth.use_camera and not self.cfg.depth.ignore_vision:
                window_name = "Depth Image"
                cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
                cv2.imshow("Depth Image", self.depth_buffer[self.lookat_id, -1].cpu().numpy() + 0.5)
                cv2.waitKey(1)

    def step_external_assets(self, **kwargs):
        pass
    ...
    def check_termination(self):
        ...
        self.time_out_buf |= self.reach_goal_cutoff
    ...
    def reset_idx(self, env_ids):
        ...
        self.timing_parameter[env_ids] = 0
        ...
        self._reset_external_assets(env_ids)
    ...
    def compute_observations(self, dx=None, dyaw=None, **kwargs):
        """
        Computes observations
        """
        imu_obs = torch.stack((self.roll, self.pitch), dim=1)

        if self.global_counter % self.cfg.env.yaw_resampling_rate == 0:
            # self.delta_yaw = self.target_yaw - self.yaw
            # self.delta_next_yaw = self.next_target_yaw - self.yaw

            self.delta_yaw = smart_delta_yaw(self.yaw, self.target_yaw)
            self.delta_next_yaw = smart_delta_yaw(self.yaw, self.next_target_yaw)

        if dyaw is not None:
            self.delta_yaw = self.delta_next_yaw = torch.zeros_like(self.target_yaw, device=self.target_yaw.device) + dyaw
        ...
        priv_latent = torch.cat(
            (self.mass_params_tensor, self.friction_coeffs_tensor, self.motor_strength[0] - 1, self.motor_strength[1] - 1), dim=-1
        )
        if self.cfg.terrain.measure_heights:
            heights = torch.clip(self.root_states[self.robot_actor_idxs, 2].unsqueeze(1) - 0.3 - self.measured_heights, -1, 1.0)
    ...
    def create_sim(self):
        ...
        print(">>> using devices", self.sim_device_id, self.graphics_device_id, self.physics_engine, self.sim_params)
        ...
        if mesh_type == "plane":
            ...
            # if self.terrain.cfg.mesh_src is not None:
            #     self._create_ground_plane()
        elif mesh_type == "custom_mesh":
            self._create_custom_terrain()
        elif mesh_type is not None:
            raise ValueError("Terrain mesh type not recognised. Allowed types are [None, plane, heightfield, trimesh]")
        print(f"Finished creating ground. Time taken {time() - start:.2f} s")
        print("*" * 80)
        self._create_envs()
    ...
    def _generate_external_assets(self):
        self.has_external_assets = False
    ...
    def _post_physics_step_callback(self):
        ...
        if self.cfg.env.observe_clock_inputs:
            self.timing_parameter = torch.remainder(self.timing_parameter + self.dt, 1.0)

            self.clock_inputs = torch.cat(
                [
                    torch.sin(2 * np.pi * self.timing_parameter).unsqueeze(1),
                    torch.cos(2 * np.pi * self.timing_parameter).unsqueeze(1),
                    torch.sin(4 * np.pi * self.timing_parameter).unsqueeze(1),
                    torch.cos(4 * np.pi * self.timing_parameter).unsqueeze(1),
                    torch.sin(np.pi * self.timing_parameter).unsqueeze(1),
                    torch.cos(np.pi * self.timing_parameter).unsqueeze(1),
                ],
                dim=-1,
            )

    def _gather_cur_goals(self, future=0) -> TensorType["num_envs", 3]:  # noqa: F821
    ...
    def _resample_commands(self, env_ids):
        ...
        # if we are on parkour terrain, we need to go forward.
        parkour_idxs = torch.where(self.env_class[env_ids] != 17)
        self.commands[env_ids[parkour_idxs], 0] = torch.abs(self.commands[env_ids[parkour_idxs], 0])
    ...
    def _compute_torques(self, actions):
        ...
        if control_type == "P":
            ...
            torques = (
                self.p_gains * (actions_scaled - self.dof_vel) - self.d_gains * (self.dof_vel - self.last_dof_vel) / self.sim_params.dt
            )
        elif control_type == "T":
            torques = actions_scaled
        elif control_type == "actuator_net":
            joint_pos_target = actions_scaled + self.default_dof_pos_all
            self.joint_pos_err = self.dof_pos - joint_pos_target
            self.joint_vel = self.dof_vel
            if self.cfg.domain_rand.randomize_motor:
                self.joint_pos_err *= self.motor_strength[0]
                self.joint_vel *= self.motor_strength[1]
    ...
    def _reset_dofs(self, env_ids):
        ...
        actor_ids = self.robot_actor_idxs[env_ids].to(device=self.device)
        actor_ids = actor_ids.to(dtype=torch.int32)

        self.gym.set_dof_state_tensor_indexed(
            self.sim, gymtorch.unwrap_tensor(self.dof_state), gymtorch.unwrap_tensor(actor_ids), len(actor_ids)
        )
    ...
    def _reset_root_states(self, env_ids):
        ...
        if self.custom_origins:
            self.root_states[self.robot_actor_idxs[env_ids]] = self.base_init_state
            self.root_states[self.robot_actor_idxs[env_ids], :3] += self.env_origins[env_ids]
            if self.cfg.env.randomize_start_pos:
                self.root_states[self.robot_actor_idxs[env_ids], :2] += torch_rand_float(
            ...
            if self.cfg.env.randomize_start_yaw:
                ...
                self.root_states[self.robot_actor_idxs[env_ids], 3:7] = quat[:, :]
            if self.cfg.env.randomize_start_y:
                self.root_states[self.robot_actor_idxs[env_ids], 1] += self.cfg.env.rand_y_range * torch_rand_float(
        ...
        self.gym.set_actor_root_state_tensor_indexed(
            ...
            gymtorch.unwrap_tensor(self.robot_actor_idxs[env_ids_int32]),
            len(env_ids_int32),
        )
    ...
    def _push_robots(self):
        ...
        self.root_states[self.robot_actor_idxs, 7:9] = torch_rand_float(
    ...
    def _update_terrain_curriculum(self, env_ids):
        ...
        dis_to_origin = torch.norm(self.root_states[self.robot_actor_idxs[env_ids], :2] - self.env_origins[env_ids, :2], dim=1)
    ...
    def _init_buffers(self):
        ...
        self.last_root_pos = torch.zeros_like(self.root_states[self.robot_actor_idxs, :3])
        ...
        if self.cfg.control.control_type == "actuator_net":
            path = os.path.join(Path(__file__).parent, self.cfg.control.actuator_net_path)
            actuator_network = torch.jit.load(path).to(self.device)
        ...
        self.timing_parameter = torch.zeros(self.num_envs, dtype=torch.float32, device=self.device, requires_grad=False)
        self.clock_inputs = torch.zeros(self.num_envs, 6, dtype=torch.float32, device=self.device, requires_grad=False)
    ...
    def _create_custom_terrain(self):
        pass
    ...
    def attach_camera(
        ...
        if (self.cfg.depth.use_camera and not self.cfg.depth.ignore_vision) or force:
    ...
    def _create_envs(self):
        ...
        """
        asset_path = self.cfg.asset.file.format(MAIN_ST_ROOT_DIR=MAIN_ST_ROOT_DIR)
        ...
        for i in tqdm(range(self.num_envs)):
            ...
            self._add_external_assets(env_handle, i)

        self.robot_actor_idxs = torch.Tensor(self.robot_actor_idxs).to(device=self.device, dtype=torch.int32)
        self._set_asset_idxs()
    ...
    def _get_env_origins(self):
        ...
        if self.cfg.terrain.mesh_type in ["heightfield", "trimesh"]:
            ...
            self.env_goals = torch.zeros(
                self.num_envs, self.cfg.terrain.num_goals + self.cfg.env.num_future_goal_obs, 3, device=self.device, requires_grad=False
            )
    ...
    def set_terrain_goal(self, goals, i, j, shift_corners=True):
        self.terrain.goals[i, j] = goals
        self.terrain.env_origins[i, j] = goals[0]

        if shift_corners:
            self.terrain.goals[i, j, :, :2] -= self.terrain.corner_shift[:2]
            self.terrain.env_origins[i, j, :2] -= self.terrain.corner_shift[:2]

        # refresh other buffers
    ...
    def _parse_cfg(self, cfg):
        ...
        if self.cfg.commands.curriculum:
            self.command_ranges = vars(self.cfg.commands.ranges)
        else:
            self.command_ranges = vars(self.cfg.commands.max_ranges)
    ...
    def _draw_goals(self):
        ...
        if not self.cfg.depth.use_camera:
            ...
            pose_robot = self.root_states[self.robot_actor_idxs[self.lookat_id], :3].cpu().numpy()
            for i in range(5):
                norm = torch.norm(self.target_pos_rel, dim=-1, keepdim=True)
                target_vec_norm = self.target_pos_rel / (norm + 1e-5)
        ...
        if hasattr(self, "feet_at_edge"):
            ...
            for i in range(4):
                pose = gymapi.Transform(
                    gymapi.Vec3(feet_pos[self.lookat_id, i, 0], feet_pos[self.lookat_id, i, 1], feet_pos[self.lookat_id, i, 2]), r=None
                )
    ...
    def _get_heights(self, env_ids=None):
        ...
        if env_ids:
            ...
            points = quat_apply_yaw(self.base_quat.repeat(1, self.num_height_points), self.height_points) + (
                self.root_states[self.robot_actor_idxs, :3]
            ).unsqueeze(1)

        # convert points from world into pixel space
        # need to add the border because heightmap is padded
        ...
        # safety check to make sure samples are within terrain
        px = torch.clip(px, 0, self.height_samples.shape[0] - 2)
        py = torch.clip(py, 0, self.height_samples.shape[1] - 2)

        # grab from heightmap (directly from terrain, these are discretized)
        heights1 = self.height_samples[px, py]
        heights2 = self.height_samples[px + 1, py]
        heights3 = self.height_samples[px, py + 1]

        # take the minimum of the three heights, probably doesn't do very much
        heights = torch.min(heights1, heights2)
        heights = torch.min(heights, heights3)

        # scale from pixels into the world units from pixels
    ...
    def _reward_tracking_goal_vel(self):
        sign = torch.where(self.commands[:, 0] == 0, 1.0, torch.sign(self.commands[:, 0]))
        ...
        #         self.commands[:, 0] + sign * 1e-5)

        rew = torch.clamp(torch.sum(target_vec_norm * cur_vel, dim=-1), -torch.abs(self.commands[:, 0]), torch.abs(self.commands[:, 0])) / (
            self.commands[:, 0] + sign * 1e-5
        )

        stay_still_idxs = torch.where(self.commands[:, 0] == 0)[0]
        rew[stay_still_idxs] = (
            -(torch.min(torch.norm(cur_vel[stay_still_idxs], dim=-1), torch.ones(len(stay_still_idxs), device=self.device)) * 2) + 1.0
        )
    ...
    def _reward_feet_stumble(self):
        ...
        rew = torch.any(
            torch.norm(self.contact_forces[:, self.feet_indices, :2], dim=2) > 4 * torch.abs(self.contact_forces[:, self.feet_indices, 2]),
            dim=1,
        )
        return rew.float()
    ...
    def _reward_feet_edge(self):
        feet_pos_xy = (
            ((self.rigid_body_states[:, self.feet_indices, :2] + self.terrain.cfg.border_size) / self.cfg.terrain.horizontal_scale)
            .round()
            .long()
        )  # (num_envs, 4, 2)
